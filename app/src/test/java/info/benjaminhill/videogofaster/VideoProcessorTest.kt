...
class VideoProcessorTest {
    ...
    fun `generateCommand produces correct ffmpeg arguments for 2x speed`() {
        ...
        val expectedFilter = "format=pix_fmts=yuv420p10le,tmix=frames=2,select='not(mod(n\\,2))',setpts=$expectedPtsModifier*PTS"
        val expectedCmd = "-y -i \"$input\" -vf \"$expectedFilter\" -c:v libx265 -crf 18 -preset slow -pix_fmt yuv420p10le -an \"$output\""
    ...
    fun `generateCommand produces correct ffmpeg arguments for 4x speed`() {
        ...
        val expectedFilter = "format=pix_fmts=yuv420p10le,tmix=frames=4,select='not(mod(n\\,4))',setpts=$expectedPtsModifier*PTS"
        val expectedCmd = "-y -i \"$input\" -vf \"$expectedFilter\" -c:v libx265 -crf 18 -preset slow -pix_fmt yuv420p10le -an \"$output\""
    ...
    fun `generateCommand produces correct ffmpeg arguments for 8x speed`() {
        ...
        val expectedFilter = "format=pix_fmts=yuv420p10le,tmix=frames=8,select='not(mod(n\\,8))',setpts=$expectedPtsModifier*PTS"
        val expectedCmd = "-y -i \"$input\" -vf \"$expectedFilter\" -c:v libx265 -crf 18 -preset slow -pix_fmt yuv420p10le -an \"$output\""
